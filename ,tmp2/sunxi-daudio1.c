...
static int sunxi_daudio_set_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	// ## hyphop ## add
	DAUDIO_DBG("%s,line:%d, F:%d m:%d f:%d i:%d\n",\
			__func__, __LINE__, 
	fmt,
...
static int __init sunxi_daudio_dev_probe(struct platform_device *pdev)
{
    ...
    DAUDIO_DBG( "%s sample_resolution: %d!\n", __func__, val.val);
    ...
    DAUDIO_DBG( "%s slot_width_select: %d!\n", __func__, val.val);
    ...
    DAUDIO_DBG( "%s pcm_lrck_period: %d!\n", __func__, val.val);
    ...
    DAUDIO_DBG( "%s pcm_lrckr_period: %d!\n", __func__, val.val);
...
ssize_t set_attr_daudio1(struct class *class, struct class_attribute *attr,
			const char *buf, size_t size)
{
	...
	if		(!memcmp(name, "mclk", 4)) {
			mclk_fs = val;
	} else if	(!memcmp(name, "sampl",5 )) {
			sample_resolution = val;
	} else if	(!memcmp(name, "pcm_lrck_", 9)) {
			pcm_lrck_period = val;
	} else if	(!memcmp(name, "pcm_lrckr", 9)) {
			pcm_lrckr_period = val;
	} else if	(!memcmp(name, "msb", 3)) {
			msb_lsb_first = val;
	} else if	(!memcmp(name, "sign", 4)) {
			sign_extend = val;
	} else if	(!memcmp(name, "slot_i", 6)) {
			slot_index = val;
	} else if	(!memcmp(name, "frame", 5)) {
			frame_width = val;
	} else if	(!memcmp(name, "tx", 2)) {
			tx_data_mode = val;
	} else if	(!memcmp(name, "rx", 2)) {
			rx_data_mode = val;
	} else if	(!memcmp(name, "slot_w", 6)) {
			slot_width_select = val;
	} else {
...
ssize_t get_attr_daudio1(struct class *class, struct class_attribute *attr, char *buf)
{
	ssize_t outsize  ;
	...
	if		(!memcmp(name, "mclk", 4)) {
			...
			val = sample_resolution;
	} else if	(!memcmp(name, "pcm_lrck_", 9)) {
			val = pcm_lrck_period;
	} else if	(!memcmp(name, "pcm_lrckr", 9)) {
			val = pcm_lrckr_period;
	} else if	(!memcmp(name, "msb", 3)) {
			val = msb_lsb_first;
	} else if	(!memcmp(name, "sign", 4)) {
			val = sign_extend;
...
static struct class_attribute daudio1_class_attrs[] = {
	...
	__ATTR(mclk_fs,				0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(sample_resolution,	0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(pcm_lrck_period,		0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(pcm_lrckr_period,	0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(msb_lsb_first,		0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(sign_extend,			0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(slot_index,			0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(frame_width,			0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(tx_data_mode,		0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(rx_data_mode,		0644,	get_attr_daudio1, set_attr_daudio1),
	__ATTR(slot_width_select,	0644,	get_attr_daudio1, set_attr_daudio1),

	__ATTR_NULL,
};

static struct class daudio1_class = {
    .name = "sunxi-daudio1",
    .owner = THIS_MODULE,
...
// ## hyphop ## add
    ...
    status = class_register(&daudio1_class);
    if (status < 0)
        pr_err("%s: status %d\n", __func__, status);
    else
        pr_info("%s success\n", __func__);
